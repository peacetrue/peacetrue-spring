...
class SortUtilsTest {
    ...
    @Test
    void comparingUnsorted() {
        Assertions.assertEquals(SortUtils.comparing(Sort.unsorted()).compare(1, 2), 0);
    }

    @Test
    void compareNullableNullsNative() {
        Assertions.assertEquals(SortUtils.compareNullable(Sort.Order.asc("id").nullsNative(), true, false), 0);
    }
}
